...
export default class AuthService {
  ...
  async signUp(user: SignUpDTO): Promise<{id:number |null; email: string | null; error: string |null}>{
    try {
      ...
      return{ id, email,error:null};
    }
    catch(error){
      return{ id:null, email: null, error:getErrorMessage(error) };
    }
  }
